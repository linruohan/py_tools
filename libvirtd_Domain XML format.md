...
### 3.2 Host bootloader 主机引导程序

采用半虚拟化技术的虚拟机管理程序通常不会模拟 BIOS，而是由主机负责启动操作系统。这可能会在主机中使用伪引导程序来提供一个界面，以便为虚拟机选择内核。例如，Xen 中的 pygrub 就是这样。Bhyve 虚拟机管理程序也使用主机引导程序，即 bhyveload 或 grub-bhyve。
...
#### 3.2.2 bootloader_args

The optional bootloader_args element allows command line arguments to be passed to the bootloader. Since 0.2.3

### 3.3 Direct kernel boot 直接内核启动

在安装新的来宾操作系统时，直接从主机操作系统中存储的内核和 initrd 文件启动通常会很有用，这样就可以将命令行参数直接传递给安装程序。这种功能通常在半虚拟化和完全虚拟化的来宾系统中都可用。
...
### 3.4 Container boot 容器启动

在使用基于容器的虚拟化方式启动域时，不再使用内核/启动映像，而是需要指定启动二进制文件的路径，使用“init”元素来实现。默认情况下，它将无参数启动。若要指定初始的 argv，请使用“initarg”元素，重复使用该元素直至所需次数。如果设置了“cmdline”元素，则它将用于提供类似于“/proc/cmdline”的功能，但不会影响 init 的 argv。
要设置环境变量，请使用“initenv”元素，每个变量对应一个元素。
若要为初始化设置自定义工作目录，请使用“initdir”元素。
若要以指定的用户或组身份运行 init 命令，请分别使用 inituser 或 initgroup 元素。这两个元素均可接受用户（或组）的 ID 或名称作为参数。将用户或组的 ID 前缀加上一个 + 号，将强制将其视为数值形式。如果不这样做，它将首先尝试作为用户或组的名称进行处理。
...
### 3.5 Common `<os>` element configuration

这些选项适用于任何形式的客机操作系统启动过程。
...
某些虚拟机管理程序允许对向客户机呈现的系统信息进行控制（例如，虚拟机可以填充 SMBIOS 字段，并通过客户机中的 dmidecode 命令进行检查）。可选的“sysinfo”元素涵盖了所有此类信息类别。自 0.8.7 版本起
...
5. uuid: Universal Unique ID number. If this entry is provided alongside a top-level uuid element (see [General metadata](https://www.libvirt.org/formatdomain.html#general-metadata)), then the two values must match.
6. sku: SKU number to identify a particular configuration.
7. family: Identify the family a particular computer belongs to.
...
If specified it is the unit such as KiB, MiB, GiB, or TiB (described in the memory element for [Memory Allocation](https://www.libvirt.org/formatdomain.html#memory-allocation)) in which size is specified, defaults to bytes.

#### 7.15.2 monitor Since 4.10.0
...
1. id: Host node id from which to allocate memory bandwidth.
2. bandwidth: The memory bandwidth to allocate from this node. The value is usually in percent (Intel) but can also be in MB/s (if resctrl is mounted with the mba_MBps option) or in 1/8 GB/s increments (AMD). The user is responsible for making sure the value makes sense on their system and configuration.
...
1. emulate: The hypervisor will provide a fake CPU cache data.
2. passthrough:The real CPU cache data reported by the host CPU will be passed through to the virtual CPU.
3. disable: The virtual CPU will report no CPU cache of the specified level (or no cache at all if the level attribute is missing).
...
1. passthrough: The number of physical address bits reported by the host CPU will be passed through to the virtual CPUs
2. emulate: The hypervisor will define a specific value for the number of bits of physical addresses via the bits attribute, (optional since 9.2.0) The number of bits cannot exceed the number of physical address bits supported by the hypervisor.
...
### 15.9 可以使用“numa”元素来指定“客户节点数组”拓扑结构。自 0.9.8 版本起
...
每个单元元素指定了一个 NUMA 单元或一个 NUMA 节点。cpus 指定了属于该节点的 CPU 或 CPU 范围。自 6.5.0 版本起，对于 qemu 驱动程序，如果模拟器二进制文件支持每个单元中的不连续 CPU 范围，那么每个单元中声明的所有 CPU 的总和将与 vcpu 元素中声明的最大虚拟 CPU 数量相匹配。这是通过将剩余的任何 CPU 填充到第一个 NUMA 单元来实现的。建议用户提供完整的 NUMA 架构，其中 NUMA CPU 的总和与 vcpus 中声明的最大虚拟 CPU 数量相匹配，以使域在 qemu 和 libvirt 版本之间保持一致。memory 指定了以千字节为单位的节点内存（即 1024 字节的块）。自 6.6.0 版本起，cpus 属性是可选的，如果省略则会创建一个无 CPU 的 NUMA 节点。自 1.2.11 版本起，可以使用额外的单位属性（参见[内存分配](https://www.libvirt.org/formatdomain.html#memory-allocation)）来定义指定内存的单位。自 1.2.7 版本起，所有单元都应在 id 属性中具有该属性，如果在代码中需要引用某个单元，则应这样做，否则单元将按递增顺序从 0 开始分配 id。将带有“id”属性的单元格与不带该属性的单元格混合是不被推荐的，因为这可能会导致不期望的行为。自 1.2.9 版本起，可选属性“memAccess”可以控制内存是被映射为“共享”还是“私有”。此设置仅适用于大页支持的内存和 nvdimm 模块。每个单元格元素都可以有一个可选的“discard”属性，该属性可针对给定的 NUMA 节点微调“丢弃”功能，详情请参阅[内存支持](https://www.libvirt.org/formatdomain.html#memory-backing)部分。可接受的值为“是”和“否”。自 4.4.0 版本起
此客户机 NUMA 规范目前仅适用于 QEMU/KVM 和 Xen 系统。

NUMA 硬件架构支持计算 NUMA 单元之间距离的概念。自 3.10.0 版本起，可以通过 NUMA 单元描述中的“距离”元素来定义 NUMA 单元之间的距离。其子元素“兄弟”用于指定相邻 NUMA 单元之间的距离值。如需了解更多详情，请参阅 ACPI（高级配置与电源接口）规范中关于系统局部性信息表（SLIT）的章节。
...
### 15.10 ACPI Heterogeneous Memory Attribute Table: ACPI 异构内存属性表
...
The <mark style="background-color: yellow；color: black">cache</mark> element then has following mandatory attributes:
...
虚拟机管理程序可能会允许对某些 CPU/机器 <mark style="background-color: yellow；color: black">功能</mark>进行 开启/关闭 的操作。
...
See [Memory Allocation](https://www.libvirt.org/formatdomain.html#memory-allocation) for more details about the unit attribute. Since 4.5.0 (QEMU only)
...
## 21 Performance monitoring events

某些平台支持对虚拟机的运行状况以及其内部执行的代码进行监控。要启用性能监控功能，您可以选择在“perf”元素中进行指定，或者通过 virDomainSetPerfEvents API 来启用。随后，可通过 virConnectGetAllDomainStats API 获取性能值。自 2.0.0 版本起
...
最后的一组 XML 元素均用于描述提供给客户域的设备。所有设备都作为“主要设备”元素的子元素出现。自 0.1.3 版本起
...
### 22.1 Hard drives, floppy disks, CDROMs

任何看起来像磁盘的设备，无论是软盘、硬盘、光盘驱动器还是虚拟化驱动程序，都通过“disk”元素进行指定。
...
<devices>
  ...
  <disk type='network' device='cdrom'>
    <driver name='qemu' type='raw'/>
    <source protocol="http" name="url_path" query="foo=ba&=flurb">
...
For bus "virtio" the model can be specified further with "virtio", "virtio-transitional" or "virtio-non-transitional". See [virtio device models](https://www.libvirt.org/formatdomain.html#virtio-device-models) for more details. Since 5.2.0

For bus "usb" the model can be specified further with usb-storage or usb-bot. There is no difference between the two models for `<disk type='disk'>`. However with usb-bot a device configured as `<disk type='cdrom'>` is properly exposed as a cdrom device inside the guest OS. Unfortunately this configuration is not ABI compatible with usb-storage and thus it can't be interchanged during migration.
...
Enables the hypervisor to connect to another process using vhost-user protocol. Requires shared memory configured for the VM, for more details see access mode for memoryBacking element (See [Memory Backing](https://www.libvirt.org/formatdomain.html#memory-backing)).
...
Enables the hypervisor to connect to a vDPA block device. Requires shared memory configured for the VM, for more details see access mode for memoryBacking element (See [Memory Backing](https://www.libvirt.org/formatdomain.html#memory-backing)).
...
使虚拟机管理程序能够连接到 FreeBSD 的 CAM 目标层（CTL），其中 CAM 是一种通用访问方法存储子系统。自 12.0.0 版本起，bhyve。
源元素具有一个必填属性“dev”，该属性指定了 CTL 设备的完整路径（例如：/dev/cam/ctl）。

---

通过“文件”、“块”和“卷”，可以使用一个或多个可选的子元素“安全标签”（参见[安全标签](https://www.libvirt.org/formatdomain.html#security-label)）来针对特定的源文件覆盖域的安全标签策略。（注意：对于“卷”类型的磁盘，只有当指定的存储卷为“文件”或“块”类型时，“安全标签”才有效）。自 0.9.9 版本起。
该源元素还可以具有“索引”属性，其含义与“存储器”中的“索引”属性相同。
源元素可能包含以下子元素：
...
The source is accessed with the permissions of the user inside the guest. This is the default accessmode if one is not specified. [More info](https://lists.gnu.org/archive/html/qemu-devel/2010-05/msg02673.html)

mapped

The source is accessed with the permissions of the hypervisor (QEMU process). [More info](https://lists.gnu.org/archive/html/qemu-devel/2010-05/msg02673.html)

squash

Similar to 'passthrough', the exception is that failure of privileged operations like 'chown' are ignored. This makes a passthrough-like mode usable for people who run the hypervisor as non-root. [More info](https://lists.gnu.org/archive/html/qemu-devel/2010-09/msg00121.html)

Since 5.2.0, the filesystem element has an optional attribute model with supported values "virtio", "virtio-transitional" or "virtio-non-transitional". See [virtio device models](https://www.libvirt.org/formatdomain.html#virtio-device-models) for more details.
...
### 22.4 Virtio-related options: 与 Virtio 相关的选项
...
#### 22.5.3 virtio-non-transitional
...
### 22.7 Device leases: 设备租赁

在使用锁管理器时，为了便于记录设备租约信息，通常会将其与虚拟机相关联。锁管理器会确保只有在成功获取租约的情况下，虚拟机才会启动。
...
### 22.8 Host device assignment

#### 22.8.1 USB / PCI / SCSI devices
...
Since 12.1.0 (QEMU and KVM only), the iommufd element can be used to enable IOMMUFD backend for VFIO device. This provides an interface to propagate DMA mappings to kernel for assigned devices. Libvirt will open the /dev/iommu and VFIO device cdev and pass associated file descriptors to QEMU.
...
#### 22.8.2 ACPI Generic Initiators
...
The value uses the standard libvirt _nodeset_ syntax (e.g. 0-3,5).

If the `<acpi>` element is omitted, no acpi-generic-initiator objects are created for the device.

#### 22.8.3 Block / character devices
...
#### 22.11.3 Userspace connection using SLIRP
...
#### 22.11.5 Generic ethernet connection
...
#### 22.11.6 Direct attachment to physical interface
...
#### 22.11.9 Teaming a virtio/hostdev NIC pair
...
#### 22.11.13 Null network interface
...
#### 22.11.14 VMWare Distributed Switch
...
#### 22.11.15 Setting the NIC model
...
#### 22.11.16 Setting NIC driver-specific options
...
#### 22.11.17 Setting network backend-specific options
...
#### 22.11.18 Overriding the target element
...
If no target is specified, certain hypervisors will automatically generate a name for the created tun device. This name can be manually specified, however the name _should not start with either 'vnet', 'vif', 'macvtap', or 'macvlan'_, which are prefixes reserved by libvirt and certain hypervisors. Manually specified targets using these prefixes may be ignored.
...
#### 22.11.19 Specifying boot order
...
#### 22.11.20 Interface ROM BIOS configuration
...
#### 22.11.21 Setting up a network backend in a driver domain
...
#### 22.11.25 Modifying virtual link state
...
#### 22.11.29 vhost-user connection
...
#### 22.11.30 vhost-user connection with passt backend
...
#### 22.11.31 Traffic filtering with NWFilter
...
The optional wait attribute, when set to yes, causes the guest to wait for an incoming VNC connection before booting. Since 11.10.0, bhyve.
...
Starts a RDP server. The port attribute specifies the TCP port number (with -1 as legacy syntax indicating that it should be auto-allocated). The autoport attribute is the new preferred syntax for indicating auto-allocation of the TCP port to use.

A non-p2p dbus graphics is required to enable the QEMU RDP support (it uses an external "qemu-rdp" helper process). The username and passwd attributes set the credentials (when they are not set, the RDP access may be disabled by the helper). Since 11.1.0

In the VirtualBox driver, the autoport will make the hypervisor pick available port from 3389-3689 range when the VM is started. The chosen port will be reflected in the port attribute. The multiUser attribute is a boolean deciding whether multiple simultaneous connections to the VM are permitted. The replaceUser attribute is a boolean deciding whether the existing connection must be dropped and a new connection must be established by the VRDP server, when a new client connects in single connection mode.

desktop

This value is reserved for VirtualBox domains for the moment. It displays a window on the host desktop, similarly to "sdl", but using the VirtualBox viewer. Just like "sdl", it accepts the optional attribute display.
...
p2p (accepts yes or no) enables peer-to-peer connections, established through virDomainOpenGraphics() APIs.

address (accepts a [D-Bus address](https://dbus.freedesktop.org/doc/dbus-specification.html#addresses)), will connect to the specified bus address.
...
Since 11.7.0 (QEMU driver only), the model element may have an optional edid attribute that can be set to on or off. If the edid attribute is not specified then the device will use its default value. Otherwise setting edid to on will expose the device EDID blob to the guest, whilst setting it to off will hide the device EDID blob from the guest. The edid attribute is only valid for model types vga, bochs, and virtio.
...
[Virtio-related options](https://www.libvirt.org/formatdomain.html#virtio-related-options) can also be set (Since 3.5.0)
...
### 22.16 Consoles, serial, parallel & channel devices
...
##### 22.16.1.4 Relationship between serial ports and consoles
...
##### 22.16.2.10 UNIX domain socket client/server
...
#### 22.18.3 Coreaudio audio backend
...
#### 22.18.7 PipeWire audio backend
...
#### 22.18.8 PulseAudio audio backend
...
Since 0.8.0, a notification is available when the watchdog fires, using the event ID VIR_DOMAIN_EVENT_ID_WATCHDOG.
...
### 22.21 Random number generator device
...
This backend connects to a source using the EGD protocol. The source is specified as a character device. Refer to the [Host interface](https://www.libvirt.org/formatdomain.html#host-interface) for more information.
...
For this backend, libvirt expects the TPM emulator to be started externally. The path to the unix socket where the emulator is listening is passed via the source element. Other backend sub-elements do not apply in this case, since they are controlled by the emulator command line.
...
For the emulator backend, the source element specifies the location of the TPM state storage. Since v10.10.0

For the external backend, it specifies the socket of the externally started TPM emulator. Since v9.0.0

This element does not work with the passthrough backend.

When specified, it is the user's responsibility to prevent files from being used by multiple VMs or emulators (swtpm will also use advisory locking). If not specified, the storage configuration is left to libvirt discretion.
...
For external backend, only type unix is supported. For emulator backend, it's possible to provide file to utilize a single file or block device where the TPM state will be stored, or dir for the directory where the files will be stored.

mode

Connection mode for the unix socket. Only connect is supported. Can be omitted.
...
The caching_mode attribute with possible values on and off can be used to turn on the VT-d caching mode (useful for assigned devices). Since 3.4.0 (QEMU/KVM and intel model only)

eim

The eim attribute (with possible values on and off) can be used to configure Extended Interrupt Mode. A q35 domain with split I/O APIC (as described in [Hypervisor features](https://www.libvirt.org/formatdomain.html#hypervisor-features)), and both interrupt remapping and EIM turned on for the IOMMU, will be able to use more than 255 vCPUs. Since 3.4.0 (QEMU/KVM and intel model only)
...
The aw_bits attribute can be used to set the address width to allow mapping larger iova addresses in the guest. Since 6.5.0 (QEMU/KVM and intel or virtio models only)

dma_translation

The dma_translation attribute with possible values on and off can be used to turn off the dma translation for IOMMU. It is useful when only interrupt remapping is required but dma translation overhead is unwanted, for example to efficiently enable more than 255 vCPUs. Since 10.7.0 (QEMU/KVM and intel model only)

passthrough

Enable passthrough. In this mode, DMA read/writes are not translated. Since 11.5.0 (QEMU/KVM and amd model only)

xtsup

Enable x2APIC mode. Useful for higher number of guest CPUs. Since 11.5.0 (QEMU/KVM and amd model only)

pciBus

The pciBus attribute notes the index of the controller that an IOMMU device is attached to. (QEMU/KVM and smmuv3 model only)
