...
class CryptoTab(BaseConfigTab):
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(crypto_frame, text='Crypto设备配置:', font=ctk.CTkFont(weight='bold')).pack(
        ...
        model_options = ctk.CTkOptionMenu(model_frame, variable=self.model_var, values=['virtio'])
        ...
        ctk.CTkLabel(props_frame, text='Crypto设备属性:', font=ctk.CTkFont(weight='bold')).pack(
            anchor='w', padx=10, pady=5
        )
        ...
        backend_options = ctk.CTkOptionMenu(
            backend_frame, variable=self.backend_var, values=['passthrough', 'emulated']
        )
        ...
        ctk.CTkLabel(backend_dev_frame, text='后端设备:', width=100).pack(
            side='left', padx=5, pady=5
        )
        self.backend_dev_entry = ctk.CTkEntry(
            backend_dev_frame, placeholder_text='例如: /dev/crypto'
        )
