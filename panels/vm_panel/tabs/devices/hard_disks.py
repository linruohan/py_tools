...
class HardDisksTab(BaseConfigTab):
    ...
    def _add_disk(self):
        ...
        HardDiskConfigDialog(dialog, self._on_disk_added, self.disk_list)
...
class HardDiskConfigDialog:
    """硬盘设备配置对话框."""

    def __init__(self, dialog, on_confirm_callback, existing_disks):
        self.dialog = dialog
    ...
    def _generate_default_target(self):
        """生成默认的目标设备名称."""
        # 收集现有的目标设备名称
        existing_targets = []
        for disk in self.existing_disks:
            target = disk.get('target_dev')
            if target:
                ...
                if target.startswith(prefix):
                    ...
                    if suffix.isdigit():
                        ...
                        if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                            ...
                            if 'nvme' not in max_numbers or ctrl_num > max_numbers['nvme']:
                                max_numbers['nvme'] = ctrl_num

        # 生成默认目标设备名称
        bus = self.bus_menu.get()
        if bus == 'nvme':
            # NVMe 设备格式: nvme0n1, nvme1n1 等
            max_ctrl = max_numbers.get('nvme', -1)
            return f'nvme{max_ctrl + 1}n1'
        elif bus == 'virtio':
            # Virtio 设备格式: vda, vdb, vdc 等
            max_num = max_numbers.get('vd', -1)
            return f'vd{chr(ord("a") + max_num + 1)}'
        elif bus == 'sata' or bus == 'scsi':
            # SATA/SCSI 设备格式: sda, sdb, sdc 等
            max_num = max_numbers.get('sd', -1)
            return f'sd{chr(ord("a") + max_num + 1)}'
        else:
            # IDE 设备格式: hda, hdb, hdc 等
            max_num = max_numbers.get('hd', -1)
            return f'hd{chr(ord("a") + max_num + 1)}'
