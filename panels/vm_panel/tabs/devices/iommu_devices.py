...
class IOMMUDevicesTab(BaseConfigTab):
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(iommu_frame, text='IOMMU设备配置:', font=ctk.CTkFont(weight='bold')).pack(
        ...
        type_options = ctk.CTkOptionMenu(
            type_frame, variable=self.type_var, values=['intel', 'amd']
        )
        ...
        ctk.CTkLabel(props_frame, text='IOMMU设备属性:', font=ctk.CTkFont(weight='bold')).pack(
            anchor='w', padx=10, pady=5
        )
        ...
        mode_options = ctk.CTkOptionMenu(
            mode_frame, variable=self.mode_var, values=['strict', 'relaxed']
        )
