...
class NetworkVLANTagTab(BaseConfigTab):
    """设置VLAN标签配置"""
    ...
    def _init_ui(self) -> None:
        ...
        # Native VLAN
        ctk.CTkLabel(frame, text='Native VLAN:', font=CTK_FONT_MAIN, width=100, anchor='w').grid(
            row=2, column=0, padx=10, pady=5, sticky='w'
        )
        self.native_vlan = ctk.CTkCheckBox(
            frame, text='Is Native VLAN', font=CTK_FONT_SMALL, command=self._trigger_change
        )
        self.native_vlan.grid(row=2, column=1, padx=5, pady=5, sticky='w')
